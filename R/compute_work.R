...
#' @param power A numeric vector representing the y-coordinates of the data points, typically power in watts.
...
#' print(auc_negative)
compute_work <- function(time, power, type = "total") {
  
  # Check that input vectors have the same length
  if (length(time) != length(power)) {
    stop("time and power vectors must have the same length.")
  }
  ...
  if (type == "total") {
    auc <- pracma::trapz(time, power)
  } else if (type == "positive") {
    # Compute positive AUC
    positive_y <- ifelse(power > 0, power, 0)
    auc <- pracma::trapz(time, positive_y)
  } else if (type == "negative") {
    # Compute negative AUC
    negative_y <- ifelse(power < 0, power, 0)
