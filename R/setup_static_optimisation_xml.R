...
#'
#' This function modifies an XML configuration file for static optimization analysis. It updates the XML file with
#' various parameters such as model file, force set files, analysis times, integrator settings, muscle physiology,
#' and other configuration parameters specific to static optimization. The modified XML file is then saved to the
#' specified setup filename.
#'
#' @param base_file Character string. The path to the base XML file to be modified (default is "setup_so_base.xml").
...
#' @param step_interval Numeric. The step interval for the static optimization analysis (default is "1").
#' @param use_model_force_set Character string. Whether to use the model's force set for optimization ("true" or "false", default is "true").
#' @param activation_exponent Numeric. The activation exponent to be used in the optimization (default is "2").
#' @param use_muscle_physiology Character string. Whether to use muscle physiology for optimization ("true" or "false", default is "true").
...
#' @param setup_filename Character string. The name of the XML file to save the modified configuration (default is "setup_so.xml").
#'
...
#' @import XML
#' @export
setup_static_optimisation_xml <- function(base_file = "setup_so_base.xml",
  ...
  ###################   Analysis set, specifics to SO    ###############################
  #start_time
  nodes = XML::getNodeSet(x, "//AnalysisSet//objects//StaticOptimization//start_time")
  XML::xmlValue(nodes[[1]]) = start_time

  #end_time
  nodes = XML::getNodeSet(x, "//AnalysisSet//objects//StaticOptimization//end_time")
  XML::xmlValue(nodes[[1]]) = end_time

  #step_interval
  nodes = XML::getNodeSet(x, "//AnalysisSet//objects//StaticOptimization//step_interval")
  XML::xmlValue(nodes[[1]]) = step_interval

  #use_model_force_set
  nodes = XML::getNodeSet(x, "//AnalysisSet//objects//StaticOptimization//use_model_force_set")
  XML::xmlValue(nodes[[1]]) = use_model_force_set

  #activation_exponent
  nodes = XML::getNodeSet(x, "//AnalysisSet//objects//StaticOptimization//activation_exponent")
  XML::xmlValue(nodes[[1]]) = activation_exponent

  #use_muscle_physiology
  nodes = XML::getNodeSet(x, "//AnalysisSet//objects//StaticOptimization//use_muscle_physiology")
  XML::xmlValue(nodes[[1]]) = use_muscle_physiology
